  ...
  <div>
    ...
    <el-card>
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-input
            @keyup.enter.native="fnGetUserList"
            @clear="fnGetUserList"
            clearable
          ...
          >
            <el-button @click="fnGetUserList" slot="append" icon="el-icon-search"></el-button>
          </el-input>
        </el-col>
        <el-col :span="8">
      ...
      <el-table :data="userlist" border stripe>
        ...
        <el-table-column label="状态">
          <template slot-scope="scope">
            <el-switch @change="fnUserStateChange(scope.row)" v-model="scope.row.mg_state"></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="190px">
          <template slot-scope="scope">
            ...
            <el-tooltip :enterable="false" effect="dark" content="分配角色" placement="top">
              <el-button @click="setRole(scope.row)" type="warning" icon="el-icon-share" size="mini"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
    ...
    return {
      ...
      addFormRules: {
        ...
        password: [
          ...
          { min: 3, max: 10, message: '长度在 6 到 18 个字符', trigger: 'blur' }
        ],
        email: [{ required: true, message: '请输入邮箱', trigger: 'blur' }],
        mobile: [
          { required: true, message: '请输入手机', trigger: 'blur' },
          { max: 11, message: '长度是11个字符', trigger: 'blur' }
        ]
      },
      // 控制修改用户对话框的显示和隐藏
  ...
  methods: {
    ...
    // 监听用户转态的改变
    async fnUserStateChange (userInfo) {
    ...
    async removeUserById (id) {
      ...
      console.log(res2)
      if (res2.meta.status === 400) {
        return this.$message.error('不允许删除admin用户!!')
      }
    ...
    async saveRolesInfo () {
      if(!this.selectedRoleId) {
        return this.$message.error('请想选择要分配的角色')
      }
